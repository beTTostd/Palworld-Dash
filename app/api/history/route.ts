...
SELECT json_object('players',(SELECT json_group_array(json_object('playerKey',player_key,'name',name,'accountName',account_name,'level',level,'hoursPlayed',ROUND(play_seconds/3600.0,2),'firstSeen',first_seen,'lastSeen',last_seen)) FROM (SELECT * FROM players ORDER BY level DESC,play_seconds DESC,name COLLATE NOCASE LIMIT 100)), 'progress',(SELECT json_group_array(json_object('playerKey',p.player_key,'name',pl.name,'accountName',pl.account_name,'sampledAt',p.sampled_at,'level',p.level,'hoursPlayed',ROUND(p.play_seconds/3600.0,4))) FROM points p JOIN players pl ON pl.player_key=p.player_key ORDER BY p.sampled_at ASC), 'events',(SELECT json_group_array(json_object('occurredAt',e.occurred_at,'playerKey',e.player_key,'name',pl.name,'type',e.event_type,'level',e.level,'previousLevel',e.previous_level)) FROM (SELECT * FROM player_events ORDER BY occurred_at DESC LIMIT 30) e JOIN players pl ON pl.player_key=e.player_key), 'metadata',(SELECT json_group_object(key,value) FROM metadata));`;
